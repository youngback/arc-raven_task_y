...
def phase_training(train_event, visual_opt, device_opt, game_opt, save_directory, global_clock=None, global_frame_log=None):
    win = visual_opt["win"]
    ...
    while clock.getTime() < duration:
        ...
        # --- 여기에 inner while (프레임 잔여 시간 채우기) ---
        i = 0
        while (clock.getTime() - t0) < FRAME_DT:
            i += 1  # no-op

        # (중요) 프레임이 1/60초 채워진 뒤 카운트
        frame_count += 1
