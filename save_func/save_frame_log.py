...
def save_frame_log_total(trial_idx, global_frame_log, save_directory, phase, total_frames=None):
    """
    Save global frame log across the entire experiment.
    """
    os.makedirs(save_directory, exist_ok=True)
    file_path = os.path.join(save_directory, f"total_frame.csv")

    with open(file_path, "w", newline='', encoding="utf-8") as csvfile:
        writer = csv.writer(csvfile)

        # Global log header
        writer.writerow(["frame_abs", "time_abs"])
        for entry in global_frame_log:
            writer.writerow([
                entry["frame_abs"],
                f"{entry['time_abs']:.4f}"
            ])
        ...
        if total_frames is None:
            total_frames = len(global_frame_log)
