...
def load_arc_trials(order_file="stim_order.txt"):
    ...
    for name in names:
        ...
        # task_id가 'Count8'처럼 문자열과 숫자가 혼합된 경우,
        # 숫자만 추출하여 홀수인지 확인합니다.
        
        # 예시: 'Count8' -> '8', 'Count7' -> '7'
        number_str = ''.join(filter(str.isdigit, task_id))
        
        # 숫자만 추출되었고, 그 숫자가 홀수인지 확인
        if number_str and int(number_str) % 2 != 0: # 짝수이면 0, 홀수이면 1이므로 0이 아닌지 확인
            test_hint_path = os.path.join(hint_dir, "question_mark.png")
        else:
            test_hint_path = os.path.join(hint_dir, f"{task_id}_hint.png")
